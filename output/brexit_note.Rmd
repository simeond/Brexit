...
One way is to use Bayesian State Space model (see Simon Jackman http://jackman.stanford.edu/papers/).  The idea is that there is an underlying preference that is sampled by the published polls.  These polls differ not only in what they tell us about the underlying latent preference, but also in their quality (eg the sample size and the methodology). So the model is weighted to take greater account of the more reliable polls. The underlying preference evolves over time.

More specifically

- $\mu_{t} \sim N(\mu_{t-1},\sigma)$ The underlying preference to 'remain' is a normally distributed random walk, where sigma is estimated from the data.  The underlying preference is estimated for each day since 1st Jan 2014.
...
The model is estimated using [stan](http://mc-stan.org/) and the code is available [here]().
